...
### 1. Yêu Cầu Hệ Thống
* **Python**: Phiên bản **3.8 trở lên** (khuyến nghị bản mới nhất).
* **Tkinter**: Thư viện giao diện chuẩn của Python.
  * Trên **Windows**: Thư viện này được cài đặt sẵn cùng với Python.
  * Trên **macOS/Linux**: Có thể cần cài đặt thêm thông qua trình quản lý gói của hệ điều hành.
* **Pillow (PIL)**: Thư viện xử lý ảnh để phục vụ tính năng lưu ảnh chụp giao diện dạng `.png`.

---

### 2. Hướng Dẫn Chi Tiết Các Bước Chạy

#### Bước 1: Kiểm tra Python
Mở Terminal/Command Prompt và chạy lệnh sau để kiểm tra xem Python đã được cài đặt đúng cách hay chưa:
```bash
python --version
# hoặc
python3 --version
...
#### Bước 3: Cài đặt thư viện Pillow
Cài đặt thư viện Pillow cần thiết để xuất file ảnh đồ thị:
* **Trên Windows / macOS / Linux:**
  ```bash
  pip install Pillow
  # Hoặc nếu dùng môi trường ngoài venv:
  python -m pip install Pillow
  ```

#### Bước 4: Cài đặt bổ sung Tkinter (Chỉ dành cho macOS / Linux nếu bị thiếu)
Nếu khi chạy chương trình gặp lỗi `ModuleNotFoundError: No module named 'tkinter'`, hãy cài đặt bổ sung:
* **Ubuntu/Debian:**
  ```bash
  sudo apt-get update
...
Chạy file `main.py` ở thư mục gốc để mở giao diện chương trình:
* **Trên Windows:**
  ```bash
  python main.py
  ```
...
* **Lỗi `ModuleNotFoundError: No module named 'PIL'`**: Cho biết thư viện `Pillow` chưa được cài đặt hoặc bạn đang chạy Python ở môi trường khác với môi trường đã chạy lệnh `pip install`. Hãy đảm bảo đã kích hoạt môi trường ảo `venv` trước khi cài đặt và chạy ứng dụng.
* **Lỗi `ModuleNotFoundError: No module named 'tkinter'`**: Xem lại hướng dẫn ở **Bước 4** để cài đặt thư viện giao diện cho hệ điều hành của bạn.
* **Lỗi hiển thị giao diện bị mờ/lệch trên màn hình độ phân giải cao (High DPI) trên Windows**: Hãy chạy chương trình trực tiếp bằng Python Command Line chính thức, hạn chế chạy trong một số terminal tích hợp không hỗ trợ DPI scaling tốt.
